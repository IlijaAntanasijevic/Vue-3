...
export default {
  ...
  methods: {
  async fetchData(){
    const responseGenres = await axios.get("genre/movie/list?language=en")
    this.dataGenresMoviesAPI = responseGenres;
    const responsePopularMovies = await axios.get("movie/popular?language=en-US&page=1")
    this.dataAPI = responsePopularMovies  
  },
  getGenre(id){
    let tmp = "";
...
async mounted(){
  ...
  this.dataAPI = this.dataAPI.data.results
  console.log(this.dataAPI);
  // console.log(this.dataGenresAPI.data);
 // console.log(this.dataUpcoming);
